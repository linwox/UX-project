  ...
  <div class="bg-[url('../assets/headerBakgrund.svg')] bg-cover bg-center h-screen">
    <div class="flex flex-col justify-center items-center h-screen space-y-4">
      <div class="flex flex-col h-screen w-screen justify-center items-center gap-5">
        ...
        <div class="carousel relative w-80 h-2/3 rounded-box md:w-1/2 bg-base-100 shadow-xl border-slate border-2">
          <div v-for="person of visiblePersons" :key="person.id"
            class="carousel-item w-full flex flex-col items-center justify-center">
            <div v-if="!person.buttonPressed" class="relative w-full h-full flex flex-col items-center justify-center">
              <img :src="person.imageUrl" alt="Politician" class="w-full h-full object-cover rounded-2xl z-0" />
              <div
                class="w-full h-72 bg-gradient-to-t from-black via-transparent absolute bottom-0 left-0 right-0 rounded-2xl z-10">
              </div>
              <p class="text-3xl text-white font-bold absolute bottom-28 left-8 z-20">{{ person.firstName }} {{
                person.age }}</p>
              ...
              <div class="flex items-end justify-center absolute bottom-6 left-6 right-6 z-20">
                <div class="card-actions space-x-36 flex justify-center">
                  <button @click="handleButtonClick(ministerPost, person.id)"
